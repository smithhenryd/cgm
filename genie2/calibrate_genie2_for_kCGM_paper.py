...
import secstruct
from cgm.cgm import calibrate_relaxed, calibrate_reward, log_p_chunked
from cgm.utils import CheckpointEveryN, clone_network
...
def wandb_logger(
    ...
    logs = {
        "regularization_loss": logs["kl_loss"],
...
def save_sample_outputs(
    ...
    with torch.no_grad(), tqdm(total=n_samples, desc="Sampling") as pbar:
        while sample_idx < n_samples:
            ...
            for batch_sample, lp_model, lp_base in zip(
                ...
                pdb_path = os.path.join(out_dir, f"sample_{sample_idx}.pdb")
                secstruct.save_pdb(x0, pdb_path)
                alpha_helix, beta_strand = secstruct.sec_struct_frac(pdb_path)[:2]
                rows.append(
                    {
                        "sample_idx": sample_idx,
                        "pdb_path": pdb_path,
                        "alpha_helix": float(alpha_helix),
                        "beta_strand": float(beta_strand),
    ...
    pd.DataFrame(rows).to_csv(
        os.path.join(out_dir, "sample_secstruct.csv"), index=False
    )
...
def main():
    ...
    parser.add_argument("--wandb_project", type=str, default="genie-finetune")
    ...
    if epochs > 0:
        ...
        ckpt_fn = CheckpointEveryN(ckpt_dir=ckpt_dir, N=epochs)
    else:
        ckpt_fn = None
    ...
    if N_samples > 0:
        ...
        save_sample_outputs(
            ...
            out_dir=out_dir,
            sample_batch_size=(
                max_sample_batch if max_sample_batch is not None else batch_size
            ),
            batch_chunks=batch_chunks,
            samp_chunks=samp_chunks,
        )
