...
class BaseSampler(ABC):
	...
	def predict_noise(self, params):
		...
		Args:
			params:
				A dictionary of sampling parameters. Required parameters include
					...
					-	step: time step
					- 	atom_pos: batched atom positions [B, N, 3]
					-	num_samples: number of samples to generate (in a batch)

		Returns:
			z_pred:
				Predicted noise of shape [B, N, 3]
		"""

		# Create features
		features = convert_np_features_to_tensor(
			batchify_np_features([
				create_empty_np_features([params['length']])
				for _ in range(params['num_samples'])
			]),
			self.device
		)

		assert params['atom_pos'].shape == features['atom_positions'].shape

		# Create frames
		trans = params['atom_pos']
		...
		if len(params['step'].shape) == 1:
			assert params['step'].shape[0] == params['num_samples']
			timesteps = torch.Tensor(params['step']).int().to(self.device)
		else:
			timesteps = torch.Tensor([params['step']] * params['num_samples']).int().to(self.device)
