...
def rot_to_quat(
    ...
    rot_ = [[rot[..., i, j] for j in range(3)] for i in range(3)]
    [[xx, xy, xz], [yx, yy, yz], [zx, zy, zz]] = rot_
    ...
    try:
        ...
        print("Batch eigh failed, repeating in loop")
        vectors = torch.stack([torch.linalg.eigh(k[i])[1] for i in
            range(k.shape[0])])
    return vectors[..., -1]
