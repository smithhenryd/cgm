...
def load_pretrained_model(rootdir, name, epoch, map_location=None):
	...
	Args:
		...
		epoch:
			Checkpoint epoch.
		map_location:
			Device onto which to load the pretrained model.
	...
	# load model
	print(f"Loading checkpoint: {ckpt_filepath} (map_location={map_location})")

	return Genie.load_from_checkpoint(ckpt_filepath, map_location=map_location, config=config)