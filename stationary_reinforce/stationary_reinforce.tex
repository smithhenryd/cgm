...
\begin{enumerate}
    ...
    \item update $\theta$ by taking a step $\theta \leftarrow \theta - \alpha\,\hat U(\theta, x_{1:M})$, where
    \[
...
\begin{itemize}
    \item Ignoring for now the bias correction, the squared error corresponds to choosing $w(h) = h - h^*$, so $w(0) = -h^*$ and $w(1) = 1 - h^*$, and $\eta(y) = 2(y - h^*)$.
    \item Intuition: if $y < h^*$ then $\eta(y) < 0$, so $w(h(x_m))\eta(y)$ is negative for $h(x_m)=1$ and positive for $h(x_m)=0$. The step $\theta \leftarrow \theta - \alpha\hat U$ therefore increases the likelihood of samples with $h(x_m)=1$ and decreases it for $h(x_m)=0$, pushing $\mathbb{E}_\theta[h(x)]$ toward $h^*$.
    \item We also add a bias correction so that $\hat U$ is an unbiased estimator of the negative gradient of the squared error, $\nabla_\theta (\mathbb{E}_\theta[h(x)] - h^*)^2$, so the step reduces the squared calibration error in expectation. Deriving the bias correction is possible because of a bias-variance decomposition special to the squared error.
    \item However, a limitation of this loss is that the violation doesn't become small enough. A reason is that $\eta(y)$ scales down the magnitude of the expected gradient when $\mathbb{E}_\theta[h(x)]$ is near $h^*$, and so optimization stops short of satisfying the constraint.
\end{itemize}
...
\]
This resembles an gradients of the absolute loss loss that we might hope this update to better guarantee exact satisfaction of the constraint. But it is not clear how to make this update unbiased for $\nabla_\theta |\mathbb{E}_\theta[h(x)]-h^*|$. And in particular it has the disadvantage that the value of the gradient estimator will almost surely be identical for every $h^* \in (0, \frac{1}{M})$. Consequently, the approach will not be able to distinguish frequencies of rare events with probability below $\frac{1}{M}$.
...
\end{align*}
Two identities follow from $\mathbb{E}_\theta[h(x)] = h^*$. First, $\mathbb{E}_\theta[y] = h^*$, so by the law of total expectation $\alpha\beta + (1-\alpha)\gamma = h^*$, giving $\gamma = \frac{h^*-\alpha\beta}{1-\alpha}$. Second, the score has zero mean, $\mathbb{E}_\theta[\nabla_\theta \log p_\theta(x)] = 0$, which gives $h^* s_1 + (1-h^*)s_0 = 0$, so $s_1 = -\frac{1-h^*}{h^*}s_0$.

Substituting $s_1$ and grouping by $w(1)$ and $w(0)$:
\begin{align*}
    &= w(1)\frac{1-h^*}{h^*}\bigl[\alpha\beta - (1-\alpha)\gamma\bigr]s_0 \;+\; w(0)\bigl[-\alpha(1-\beta) + (1-\alpha)(1-\gamma)\bigr]s_0.
\end{align*}
Using $(1-\alpha)\gamma = h^*-\alpha\beta$ and $(1-\alpha)(1-\gamma)=1-\alpha-h^*+\alpha\beta$, the bracketed coefficients simplify to $2\alpha\beta - h^*$ and $1-2\alpha+2\alpha\beta-h^*$, respectively. Factoring out $(-s_0)$:
\begin{align*}
    &= \left[w(1)\frac{1-h^*}{h^*}(h^*-2\alpha\beta) + w(0)\bigl(2\alpha(1-\beta)-(1-h^*)\bigr)\right](-s_0).
\end{align*}
...
The normalization $D$ is chosen so that $w(1) - w(0) = 1$, matching the absolute-error gradient scaling for $p$ far from $h^*$, for any $M$.
The weights are also symmetric under $h^* \leftrightarrow 1-h^*$ (the roles of $w(0)$ and $w(1)$ swap), and recover $w(1) \to h^*$, $w(0) \to -(1-h^*)$ as $M \to \infty$.
...
The top row shows the expected update direction $\mathbb{E}_p[\hat{U}]$ as a function of the model parameter $p$, for three target values $h^* \in \{0.01, 0.1, 0.3\}$.
...
The absolute error and Stationary REINFORCE estimators have implied losses that are approximately $V$-shaped near $h^*$, with gradients that remain bounded away from zero, leading to sharper convergence near the constraint, but with significant bias (especially for small $h^*$).
Each batch-size-dependent estimator is shown for two batch sizes: $M=8$ (solid) and $M=64$ (dotted); the squared error estimator does not depend on batch size and is shown once.
...
\begin{figure}[h]
    ...
    \caption{%
        ...
        Top: $\mathbb{E}_p[-\hat{U}]$ vs.\ model parameter $p$ for target values $h^* \in \{0.01, 0.1, 0.3\}$.
        Bottom: implied loss $V(p) = \int_0^p \mathbb{E}_q[-\hat{U}]\,/\,(q(1-q))\,\mathrm{d}q$, shifted so $V(h^*)=0$.
        The vertical gray line marks $p = h^*$.
        Absolute error and Stationary REINFORCE are shown for $M=8$ (solid) and $M=64$ (dotted); squared error is batch-size independent.
